...
function randomcolor(event, api){
    ...
    return api.changeThreadColor(color[Math.floor(Math.random() * color.length)], event.threadID, (err) => (err) ? outMsg(('error'), event.threadID, event.messageID) : '');
        }
module.exports = {
    randomcolor,
    config
